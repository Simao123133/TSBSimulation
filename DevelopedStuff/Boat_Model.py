...
def asind(sin_ang):
    return np.arcsin(sin_ang)*180/np.pi

def atand(tan_ang):
    return np.arctan(tan_ang)*180/np.pi

def boat_update(Y, t, inputs):
    ...
    #inputs
    Act_front_left = inputs[0]
    Act_front_right = inputs[1]
    Act_rear = inputs[2]
    Rpms_motor = 1000*inputs[3]
    Rudder = inputs[4]
    ...
    uvw = np.array([u, v, w])
    X_dot = np.matrix.transpose(R)@uvw
    z_dot = X_dot[2]
    ...
    C_CoP = h_C/2 - z_C_no_roll 
    LS_CoP = h_LS/2 - z_LS
    RS_CoP = h_RS/2 - z_RS

    #Body angular velocities to euler
    Q = np.array([[1, sind(Roll)*tand(Pitch), cosd(Roll)*tand(Pitch)],
    ...
    #Effective angle has to account for both the actuator's angle and the incoming water velocity
    ...
    #Dynamic pressure at each component
    ...
    D_RF=D_induced_RF+D_skf_RF+D_form_RF_right+D_interference_RF+Dw_RF
    ...
    X = X_LF + X_RF + X_ReF + X_LS + X_RS + X_C + X_Prop + D_b_H[0]
    Y = Y_LF + Y_RF + Y_ReF + Y_LS + Y_RS + Y_C + Y_Prop + D_b_H[1]
    Z = Z_LF + Z_RF + Z_ReF + Z_LS + Z_RS + Z_C + D_b_H[2]
    ...
    #Balance of forces in the body frame
    pqqr = np.array([p, qq, r])*np.pi/180 

    uvw_dot = (F + R@W_inertial + R@BUOY_inertial)/m -np.cross(pqqr, uvw)

    #Balance of moments
    J = np.array([[Ixx, 0, Ixz], [0, Iyy, 0], [Ixz, 0, Izz]])
    invJ = np.array([[Izz/(Ixx*Izz - Ixz**2), 0, -Ixz/(Ixx*Izz - Ixz**2)],
                     [0, 1/Iyy, 0], 
                     [-Ixz/(Ixx*Izz - Ixz**2), 0, Ixx/(Ixx*Izz - Ixz**2)]])

    W_dot = invJ@(Tau - np.cross(pqqr, J@pqqr))

    p_dot = W_dot[0]*180/pi
    q_dot = W_dot[1]*180/pi
    r_dot = W_dot[2]*180/pi
...
def boat_update_trim(t, Y, inputs, params):
