...
def boat_eq(t, x, u, params):
    ...
    #z_cm = z_cm_no_roll/cosd(Roll)   #boat's cm heave   
    ...
    V_LS = np.array([u_s,v,w]) + np.cross(W,R_LS)
    #Right Strut
    R_RS = np.array([dist_S_cm_x,dist_FF_cm_y,RS_CoP])
    V_RS = np.array([u_s,v,w]) + np.cross(W,R_RS)
    #Column
    R_C = np.array([-dist_C_cm_x,0,C_CoP])
    V_C = np.array([u_s,v,w]) + np.cross(W,R_C)
    #Left foil
    R_LF = np.array([dist_S_cm_x,-dist_FF_cm_y,dist_FF_cm_z])
    V_LF = np.array([u_s,v,w]) + np.cross(W,R_LF)
    #Right foil 
    R_RF = np.array([dist_S_cm_x,dist_FF_cm_y,dist_FF_cm_z])
    V_RF = np.array([u_s,v,w]) + np.cross(W,R_RF)
    #Rear foil
    R_RE = np.array([-dist_C_cm_x,0,dist_ReF_cm_z])
    V_ReF = np.array([u_s,v,w]) + np.cross(W,R_RE)
    ...
    speed_cm = np.linalg.norm(np.array([u_s,v,w]))
    ...
    #Wave Drag #Took square out to get polynomial order 5
    ...
    D_RF=D_induced_RF+D_skf_RF+D_form_RF_right+D_interference_RF+Dw_LF
    ...
    #Body frame forces
    L_b_LF = L_LF*np.array([sind(aa_LF),0,-cosd(aa_LF)])
    L_b_RF = L_RF*np.array([sind(aa_RF),0,-cosd(aa_RF)])
    ...
    D_b_LF = D_LF*np.array([-cosd(aa_LF),0,-sind(aa_LF)])
    D_b_RF = D_RF*np.array([-cosd(aa_RF),0,-sind(aa_RF)])
    ...
    D_b_H = D_H*np.array([-u_s/speed_cm,-v/speed_cm,-w/speed_cm])
    ...
    X_LF = L_b_LF[0] + D_b_LF[0]
    Y_LF = 0
    Z_LF = L_b_LF[2] + D_b_LF[2]
    X_RF = L_b_RF[0] + D_b_RF[0]
    Y_RF = 0
    Z_RF = L_b_RF[2] + D_b_RF[2]
    ...
    #Sum of body forces in body axis
    X = X_LF + X_RF + X_ReF + X_LS + X_RS + X_C + X_Prop + W_b[0] + BUOY_b[0] + D_b_H[0]
    Y = Y_LF + Y_RF + Y_ReF + Y_LS + Y_RS + Y_C + Y_Prop + W_b[1] + BUOY_b[1] + D_b_H[1]
    Z = Z_LF + Z_RF + Z_ReF + Z_LS + Z_RS + Z_C + W_b[2] + BUOY_b[2]+ D_b_H[2]
    
    #Sum of moments in body axis
    L = -Y_LS*LS_CoP-Z_LF*dist_FF_cm_y-Y_RS*RS_CoP +Z_RF*dist_FF_cm_y-Y_C*C_CoP - Y_Prop*(dist_ReF_cm_z-0.2) #dist z of propeller 
    M = X_LF*dist_FF_cm_z+X_LS*LS_CoP + X_RF*dist_FF_cm_z+X_RS*RS_CoP-Z_LF*dist_FF_cm_x-Z_RF*dist_FF_cm_x+Z_ReF*dist_C_cm_x+X_ReF*dist_ReF_cm_z+X_C*C_CoP+X_Prop*(dist_FF_cm_z)
    N = X_LF*dist_FF_cm_y+X_LS*dist_FF_cm_y-X_RF*dist_FF_cm_y-X_RS*dist_FF_cm_y+Y_LF*dist_FF_cm_x+Y_LS*dist_FF_cm_x+Y_RF*dist_FF_cm_x+Y_RS*dist_FF_cm_x-Y_ReF*dist_C_cm_x-Y_C*dist_C_cm_x-Y_Prop*dist_C_cm_x
    ...
    u_dot = X/m-qq*w+r*v
    v_dot = Y/m+p*w-u_s*r
    w_dot = Z/m-p*v+qq*u_s

    V_I_dot = np.linalg.inv(R)@np.array([u_s, v, w])

    #Balance of moments
    p_dot = (L/Ixx+Ixz*N/(Ixx*Izz)-Ixz**2/(Ixx*Izz)*qq*r+Ixz/Ixx*(1+(Ixx-Iyy)/Izz)*p*qq+(Iyy-Izz)/Ixx*qq*r)/(1-Ixz**2/(Ixx*Izz))
    r_dot = N/Izz+Ixz/Izz*(p_dot-qq*r)+(Ixx-Iyy)/Izz*p*qq
    q_dot = M/Iyy+Ixz/Iyy*(r**2-p**2)+(Izz-Ixx)/Iyy*r*p
    ...
    dydt = [u_dot, w_dot, q_dot, Pitch_dot, Z_dot, v_dot, p_dot, r_dot, Roll_dot, Yaw_dot]

    return dydt

boat = ct.NonlinearIOSystem(
    boat_eq, None, inputs=('Act_front_left', 'Act_front_right', 'Act_rear', 'Rpms_motor', 'Rudder'), outputs=('u', 'w', 'q', 'pitch', 'z', 'v', 'p', 'r', 'roll', 'yaw'),
    states=('u', 'w', 'q', 'pitch', 'z', 'v', 'p', 'r', 'roll', 'yaw'), name='boat')

eqpt, ueq = ct.find_eqpt(boat, [7,0,0,0,-0.4,0,0,0,0,0], [1.55, 1.55, 2.02, 4780, 0])
